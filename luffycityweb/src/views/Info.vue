    ...
    <div class="main">
      <div class="course-info">
        ...
        <div class="wrap-right">
          ...
          <p class="data">
            {{ course.info.students }}人在学&nbsp;&nbsp;&nbsp;&nbsp;课程总时长：{{ course.info.pub_lessons }}课时/{{ course.info.lessons }}课时&nbsp;&nbsp;&nbsp;&nbsp;难度：{{
              course.info.get_level_display
            }}
          </p>
          <div class="sale-time" v-if="course.info.discount.type">
            <p class="sale-type">{{ course.info.discount.type }}</p>
            <p class="expire" v-if="course.info.discount.expire > 0">
              距离结束：仅剩 {{ parseInt(course.info.discount.expire / 86400) }}天 {{ fill0(parseInt((course.info.discount.expire / 3600) % 24)) }}小时
              {{ fill0(parseInt((course.info.discount.expire / 60) % 60)) }}分
              <span class="second">{{ fill0(parseInt(course.info.discount.expire % 60)) }}</span>
              秒
            </p>
          </div>
          <div class="sale-time" v-if="!course.info.discount.type">
          ...
          <div class="buy">
            ...
            <div class="add-cart">
              <img src="../assets/cart-yellow.svg" alt="" />
              加入购物车
            </div>
          </div>
        </div>
      ...
      <div class="course-tab">
        <ul class="tab-list">
          ...
          <li :class="course.tabIndex === 2 ? 'active' : ''" @click="course.tabIndex = 2">
            课程章节
            <span :class="state.tabIndex !== 2 ? 'free' : ''" v-if="course.con_free_study">(试学)</span>
          </li>
      ...
      <div class="course-content">
        <div class="course-tab-list">
          ...
          <div class="tab-item" v-if="course.tabIndex === 2">
            ...
            <div class="chapter-item" v-for="chapter in course.chapter_list" v-bind:key="chapter">
              <p class="chapter-title">
                <img src="../assets/1.svg" alt="" />
                第{{ chapter.orders }}章·{{ chapter.name }}
              </p>
              <div class="chapter-title" style="padding-left: 2.4rem" v-if="chapter.summary" v-html="chapter.summary"></div>
              <ul class="lesson-list">
                <li class="lesson-item" v-for="lesson in chapter.get_lesson_list" v-bind:key="lesson">
                  <p class="name">
          ...
          <div class="tab-item" v-if="course.tabIndex === 3">用户评论</div>
          <div class="tab-item" v-if="course.tabIndex === 4">常见问题</div>
        </div>
...
import { ElMessage } from 'element-plus'; // 错误信息渲染
